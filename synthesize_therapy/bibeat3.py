...
ear_choice = int(input("이명이 들리는 귀를 선택하세요 (왼쪽: 1, 오른쪽: 2, 양쪽:3): "))
...
if sound_choice == 1:
    ...
    white_noise_data = np.array(white_noise.get_array_of_samples())
    bandpass_white_noise = bandpass_filter(white_noise_data, notch_freq * 0.9, notch_freq * 1.1, fs)
    bandpass_white_noise_audio = AudioSegment(bandpass_white_noise.tobytes(), frame_rate=fs, sample_width=white_noise.sample_width, channels=1)
    combined_audio = filtered_audio.overlay(bandpass_white_noise_audio)

elif sound_choice == 3:
    # 핑크 노이즈 생성 및 볼륨 조절
    duration_ms = len(filtered_audio)  # 기존 오디오와 동일한 길이
    pink_noise_data = generate_pink_noise(duration_ms, fs)
    bandpass_pink_noise = bandpass_filter(pink_noise_data, notch_freq * 0.9, notch_freq * 1.1, fs)
    pink_noise = AudioSegment(bandpass_pink_noise.tobytes(), frame_rate=fs, sample_width=pink_noise_data.dtype.itemsize, channels=1)
...
# 결과 파일로 저장
combined_audio.export("C:\\Users\\taeso\\OneDrive\\바탕 화면\\test_complete22.mp3", format="mp3")
